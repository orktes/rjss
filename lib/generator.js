...
Generator.prototype = {
  ...
  createFunction: function (code, attributes) {
    var localVariables = ["$rel"].concat(attributes);

    var macroCode = "function fn" + (++this._fnIds) + "(" + localVariables.join(', ') + ") {";
    macroCode += code;
    macroCode += "}";

    var outsideVariables = [];
      ...
      if (node.type === "Identifier") {
       if (node.parent && (
         ...
         if (node.parent.type !== 'MemberExpression' || node.parent.object === node) {
            ...
            if (outsideVariables.indexOf(variable) === -1) {
              outsideVariables.push(variable);
            }
    ...
    _.each(outsideVariables, function (variable) {
      if (variable.indexOf('.') === -1) {
        ref[variable] = this.file.compileDeclaration(variable, true);
      } else {
        var parts = variable.split('.');
        var namedImport = parts.shift();
        variable = parts.join('.');
        ...
        if (!(file = this.file.namedImports[namedImport])) {
          // TODO better error
          throw new Error("No import with name " + namedImport);
        }
    ...
    return function () {
      var args = [ref].concat(Array.prototype.slice.call(arguments));
