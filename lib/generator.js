  ...
  _generateDeclaration: function (declaration, includeValues) {
    switch(declaration.type) {
      ...
      case "FUNC":
        ...
        if (declaration.name.indexOf('.') === -1) {
          globalScope = this._resolveGlobalVariable(declaration.name);
          fn = (includeValues ? (("typeof values." + declaration.name + " !== 'undefined' && values." +
                 declaration.name) + " || ") : '') + globalScope;
        } else {
          parts = declaration.name.split('.');
          name = parts.shift();
          globalScope = this._resolveGlobalVariable(name);
          property = parts.join('.');

          fn = (includeValues ? (("typeof values." + name + " !== 'undefined' ? values." +
                 name) + " : ") : '') + globalScope;
